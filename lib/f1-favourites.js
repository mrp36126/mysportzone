...
async function buildF1Favourites({ preferCache = false, persist = false } = {}) {
  ...
  const [raceResults, sprintResults, qualifyingRows, practiceRows, weatherScore, sentimentScores] = await Promise.all([
    fetchRaceResults().then(rows => rows.length ? rows : localRaceResults).catch(() => localRaceResults),
    Promise.resolve(localSprintResults),
    fetchQualifyingResults(nextRace.Round).catch(() => []),
    fetchPracticeResults(nextRace.Round).catch(() => []),
...
function calculateFavourites({
  ...
  return driverPool
    .map(driver => {
      const formScore = scoreRecentForm(recentResultsByDriver.get(driver.driver_name), driver);
      const qualifyingScore = scoreQualifying(qualifyingByDriver.get(driver.driver_name), driver);
      const teamScore = teamScores.get(driver.constructor_name) ?? NEUTRAL_SCORE;
      const trackScore = scoreTrackHistory(trackByDriver.get(driver.driver_name), driver);
      const practiceScore = scorePractice(practiceByDriver.get(driver.driver_name));
      ...
      return {
        ...
        explanation: buildExplanation({
          ...
          sentimentScore,
          hasPractice: (practiceByDriver.get(driver.driver_name) || []).length > 0,
          hasQualifying: (qualifyingByDriver.get(driver.driver_name) || []).length > 0
        }),
        updated_at: new Date().toISOString()
      };
    })
...
function scoreQualifying(rows = [], driver) {
  ...
  if (recentRows.length === 0) {
    return clamp(100 - ((driver.standings_position - 1) * 3.5));
  }
...
function scorePractice(rows = []) {
  ...
  return clamp(average(rows.map(row => positionToScore(toNumber(row.Position, 20), 20))));
}
...
function weightedAverage(scores) {
  return Object.entries(SCORE_WEIGHTS).reduce((sum, [key, weight]) => {
...
async function fetchPracticeResults(round) {
  const endpoints = ['fp1', 'fp2', 'fp3'].map(session => (
    `https://api.jolpi.ca/ergast/f1/${CURRENT_SEASON}/${encodeURIComponent(round)}/${session}.json`
  ));
  const payloads = await Promise.all(endpoints.map(url => fetchJson(url).catch(() => null)));

  return payloads.flatMap(payload => {
    const races = payload?.MRData?.RaceTable?.Races || [];
    return races.flatMap(race => {
      const sessions = race.PracticeResults || race.FirstPracticeResults || race.SecondPracticeResults || race.ThirdPracticeResults || [];
      return sessions.map(result => mapResultRow(race, result));
    });
  });
}
